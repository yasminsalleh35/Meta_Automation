...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`🔍 [meta-discovery-diff] Checking for new/updated campaigns for user=${user.id}, ad_account=${ad_account_id}`);
    ...
    const existingIds = new Set((existingCampaigns || []).map(c => c.meta_campaign_id));
    ...
    console.log(`📦 Found ${allCampaigns.length} campaigns from Meta, ${existingIds.size} already in DB`);
    ...
    for (const campaign of allCampaigns) {
      ...
      const isNew = !existingIds.has(campaign.id);
      ...
      if (upsertError) {
        ...
        if (isNew) {
          ...
          const existing = existingCampaigns?.find(c => c.meta_campaign_id === campaign.id);
          if (existing && existing.name !== campaign.name) {
            updated++;
          } else {
            unchanged++;
          }
    ...
    console.log(`✅ Discovery diff complete: ${inserted} new, ${updated} updated, ${unchanged} unchanged`);
    ...
    console.error("Error in meta-discovery-diff:", error);
    return new Response(JSON.stringify({ 
